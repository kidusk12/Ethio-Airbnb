...
CREATE TABLE listings (
  ...
  house_deed_photo_url TEXT NOT NULL, -- per-listing, unlike id_document_url
  photos TEXT[] NOT NULL DEFAULT '{}',

  amenities TEXT[] NOT NULL DEFAULT '{}', -- lowercase values: 'wifi','kitchen','free_parking','washer'
  bedrooms SMALLINT NOT NULL DEFAULT 1,
  bathrooms SMALLINT NOT NULL DEFAULT 1,
  max_guests SMALLINT NOT NULL DEFAULT 1,

  price_per_night NUMERIC(10,2) NOT NULL,
  house_rules TEXT[] NOT NULL DEFAULT '{}', -- free text, presets + custom, no enum
...
CREATE TABLE payments (
  ...
  submitted_at TIMESTAMP NOT NULL DEFAULT now(), -- informational only, NOT the enforcement clock
...
CREATE TABLE transactions (
  ...
  counterparty_name TEXT NOT NULL, -- guest name (userPayment) or host name (hostPayout), snapshotted
  amount NUMERIC(10,2) NOT NULL,

  booking_id UUID NOT NULL REFERENCES bookings(id),
  performed_by_admin_id UUID NOT NULL REFERENCES users(id),
  performed_by_admin_name TEXT NOT NULL, -- snapshotted; admin name is locked post-creation but denormalized here regardless, matching the pattern
